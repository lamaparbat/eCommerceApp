...
if(isset($_POST["update"])){ 
  ...
  $update_query = "UPDATE store SET collectiontype='$cType', ptype='$pType', pname='$pName', pprice='$pPrice', img='$img', date='$date' WHERE pid = '$pid'";
   ...
   <div class="wrapper">
      ...
      <div class="content-wrapper">
         ...
         <div class="container">
           <div class="row">
             <div class="col-md-4">
              <h4 class="pt-4">Update Product &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <button class="btn btn-primary pl-2 pr-2" onclick="refresh()">Refresh Page</button></h4><hr>
               <form class="form" id="form" method="post" action="latest.php" enctype="multipart/form-data">
                 <div class="form-group">
                    <select class="form-control" id="ctype" name="cType">
                      <option>Choose Collections Type</option>
                 ...
                 <input class="form-control mb-3" type="text" name="pid" placeholder="Enter Product ID " id="pid">
                 ...
                 <div class="input-group mb-3">
                  <div class="custom-file">
                      <input type="file" name="img" class="custom-file-input" value="" id="file" onclick="select_photo()" required="">
         ...
         <!-- display all product -->
         <h4 class="pt-4 pl-3">Trending Product</h4>
         <div class="container-fluid pt-4 pr-3">
            <div class="row">
              <?php 
              $current_date = date("m");
              $select = "SELECT* FROM store WHERE date='$current_date' ORDER BY pid DESC";
              $fire = mysqli_query($con, $select) or die(mysqli_error($con));
              while($row = mysqli_fetch_assoc($fire)){
              ?>
              <div class="col-md-3">
                <div class="card card2">
                  <img src="/aroma/quantam-lite/upload/<?php echo $row['img']; ?>" class="card-img-top">
                  <div class="card-body">
                    <h6 class="card-title" style="margin-top: -15px;"><?php echo $row['pname']; ?></p>
                    <h6 class="card-title" style="margin-top: -10px;">$USD<?php echo $row['pprice']; ?></p>
                    <button class="btn btn-dark text-white" style="font-size: 13px;" onclick="update(<?php echo $row['pid']; ?>,'<?php echo $row['collectiontype']; ?>','<?php echo $row['ptype']; ?>','<?php echo $row['pname']; ?>',<?php echo $row['pprice']; ?>,'<?php echo $row['img']; ?>')">UPDATE</button>
                    <a class="btn btn-danger text-white ml-3" style="font-size: 13px;" onclick="Delete(<?php echo $row['pid']; ?>)">DELETE</a>
                  </div>
                </div>
              </div> 
              <?php } ?>
