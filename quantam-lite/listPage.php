...
//get the coupon id
if(isset($_POST["cid_btn"])){
  $cid = $_POST["cid"];
  // delete the coupons
  $query = "DELETE FROM page WHERE id='$cid'";
...
<body class="sidebar-mini fixed" style="background-color:#ebeff2;">
   <div class="wrapper">
      ...
      <div class="content-wrapper pt-5">
         ...
         <h4 class="ml-2">List Pages</h4>
         <div class="row p-1 mt-3">
           <div class="col-md-6" id="page_data">
             <!--Coupon list displaying in table -->
                <table class="table bg-white" id="table">
                  ...
                  ?>
                    <tr class="col bg-danger">
                      <td width="100px"><?php echo $row["name"]; ?></td>
                      <td width="100px"><?php echo $row["date"]; ?></td>
                      <form method="post" action="listPage.php">
                        <input type="hidden" name="cid" id="cid">
                        <input class="invisible" type="submit" name="cid_btn" id="cid_btn" style="position: absolute;">
                      </form>
                      <td width="100px"><i class="icon-eye ml-2" onclick="display(<?php echo $row["id"]; ?>)"></i></td>
                      <td width="100px">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<i class="icon-trash" onclick="delete_coupon(<?php echo $row["id"]; ?>)"></i></td>
                    </tr>
                  <?php } ?>
                  </tbody>
                </table>
           ...
           <!-- page_display on view button click -->
           <div class="col-md-6 pr-5" id="page_display"></div>
           <!-- /end of page_display -->

         </div>
         <!-- row ends -->

         <!-- float model on model view -->
         <div class="container" id="float_page_display">
           <div class="row"></div>
         </div>
         <!-- /float model on model view -->
   ...
   <script>
     ...
     function update(pid,ctype,ptype,pname,pprice,img){
      ...
      //transfer the page id to page_display.php
      function display(id){
        var display = $("#page_display").css("display")
        if(display == "none"){
          $("#page_display").css("display","block")
          $.ajax({
            url:"page_display.php",
            method:"post",
            data:{data:true, id},
            success: function(data){
              $(".content-wrapper .row #page_display").html(data);
            }
          })
        }else{
      ...
      //resize the size of table on mobile view
      $(".content-wrapper .row #page_data").css("height","80vh")
      $(".content-wrapper .row #page_data").css("overflow-y","scroll")
      var width = $(window).width()
      if(width<700){
        $(".content-wrapper .row #page_data").css("height","20vh")
        $(".content-wrapper .row #page_data").css("margin-bottom","10px")
      }
